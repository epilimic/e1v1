...
stock PrintBossPercents(client)
{
	...
	{
		if (iTankPercent)
			CPrintToChat(client, "{red}Tank{default}: {olive}[{green}%d{default}%%{olive}]", iTankPercent);
		else
			CPrintToChat(client, "{red}Tank{default}: {olive}[{red}Static {default}or {red}None{olive}]");
	}

	if(GetConVarBool(hCvarWitchPercent))
	{
		if (iWitchPercent)
			CPrintToChat(client, "{blue}Witch{default}: {olive}[{green}%d{default}%%{olive}]", iWitchPercent);
		else
			CPrintToChat(client, "{blue}Witch{default}: {olive}[{blue}Static {default}or {blue}None{olive}]");
	}
}

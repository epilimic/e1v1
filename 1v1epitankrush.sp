...
public Plugin:myinfo = {
    name = "No Tank Rush - epilimic's 1v1",
    author = "Jahze, vintik, epilimic",
    version = "1.1",
...
FreezePoints( bool:show_message = false ) {
    if ( !bTankAlive ) {
        ...
        if ( show_message ) CPrintToChatAll("{default}-{blue}e's 1v1{default}- {blue}Tank is up, distance points frozen. {green}Kill it!");
...
UnFreezePoints( bool:show_message = false ) {
    if ( bTankAlive ) {
        if ( show_message ) CPrintToChatAll("{default}-{blue}e's 1v1{default}- {blue}Tank? {green}What tank? {blue}Points unfrozen.");

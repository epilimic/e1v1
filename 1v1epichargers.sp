...
new Handle: hKillTrigger = INVALID_HANDLE;                      // when to kill charger, flags (1: after first impact, 2: after first slam, 0: never)
new Handle: hReportAllHits = INVALID_HANDLE;                    // whether to report all melee hits to client
...
public Plugin:myinfo = 
{
    name = "1v1 Chargers",
    author = "Tabun",
    description = "epilimic's 1v1 chargers, also for stabs",
    version = "0.1c",
    url = "nope"
}
...
public OnPluginStart()
{
    ...
    hDmgPunch = CreateConVar("sm_1v1charge_dmg_punch",          "0",    "Damage per (normal) charger punch.", FCVAR_PLUGIN, true, 0.0);
    hDmgFirst = CreateConVar("sm_1v1charge_dmg_firstpunch",     "5",    "Damage for first charger punch (in its life).", FCVAR_PLUGIN, true, 0.0);
    hDmgSmash = CreateConVar("sm_1v1charge_dmg_impact",         "5",    "Damage for impact after a charge.", FCVAR_PLUGIN, true, 0.0);
    hDmgStumble = CreateConVar("sm_1v1charge_dmg_stumble",      "2",    "Damage for impact after a charge.", FCVAR_PLUGIN, true, 0.0);
    hDmgPound = CreateConVar("sm_1v1charge_dmg_pound",          "0",    "Damage for pounds after charge/collision completed.", FCVAR_PLUGIN, true, 0.0);
    hKillTrigger = CreateConVar("sm_1v1charge_killcharger",     "2",    "When to kill charger (1 = after first impact, 2 = after first groundpound).", FCVAR_PLUGIN, true, 0.0);
    hReportAllHits = CreateConVar("sm_1v1charge_reportallhits", "0",    "Whether to report all melee hits on charger to the survivor.", FCVAR_PLUGIN, true, 0.0, true, 1.0);
...
public PlayerHurt_Event(Handle:event, const String:name[], bool:dontBroadcast)
{
    ...
    if (GetClientTeam(attacker) != TEAM_SURVIVOR || GetClientTeam(victim) != TEAM_INFECTED) { return; } 
    ...
    if (damagetype & DMG_CLUB || damagetype & DMG_SLASH) {
        ...
        if (health == 0 && bChargerCharging[victim]) {
            // it's a melee kill & a level
            CPrintToChatAll("{default}-{blue}e's 1v1{default}- {olive}%N {blue}got a level for {green}%d {blue}damage {olive}[{blue}in the {green}%s{olive}]", attacker, damage, hitGroupName(hitgroup));
            
        } else if (GetConVarBool(hReportAllHits)) {
            // not a level, but let's show it anyway
            CPrintToChatAll("{default}-{blue}e's 1v1{default}- {olive}%N {blue}hit for {green}%d {blue}damage {olive}[{blue}in the {green}%s{olive}]", attacker, damage, hitGroupName(hitgroup));
        }
    }
}
...
public Action:OnTakeDamage(victim, &attacker, &inflictor, &Float:damage, &damageType, &weapon, Float:damageForce[3], Float:damagePosition[3])
{
    ...
    new iKillTrigger = GetConVarInt(hKillTrigger);
    ...
    if (damage == CHARGER_DMG_DEFAULT)
    {
        if (damageForce[0] == 0.0 && damageForce[1] == 0.0 && damageForce[2] == 0.0) {
            ...
            if (iKillTrigger == 1) {
                ...
                CPrintToChatAll("{default}-{blue}e's 1v1{default}- {olive}%N {blue}had {green}%d {blue}health remaining.", attacker, remaining_health);
                CreateTimer(CHARGER_KILL_TIME, destroyCharger, attacker);
            }
    ...
    return Plugin_Changed;
    */
    CPrintToChatAll("{default}-{blue}e's 1v1{default}- {green}warning, charger doing a type of damage it shouldn't! infl.: [%s] type [%d] damage [%.0f] force [%.0f %.0f %.0f]", classname, damageType, damage, damageForce[0], damageForce[1], damageForce[2]);
    return Plugin_Handled;
}

public Action:destroyCharger(Handle:timer, any:client)
{

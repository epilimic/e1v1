...
CreateBoomer() {
	new dude = CreateFakeClient("Boomer");
	
//	if (dude == 0) {
//		PrintToChatAll("tried to create boomer but got 0 index");
//		return;
//	}
...
public Action:player_Death(Handle:event, const String:name[], bool:dontBroadcast) {
	if (IsFakeClient(GetClientOfUserId(GetEventInt(event, "userid")))) {
		KickClient(GetClientOfUserId(GetEventInt(event, "userid"))
);
	}
}
...
public OnEntityCreated(entity, const String:classname[])
{
	if (tank && StrEqual(classname, "tank_rock", true) && GetEntProp(entity, Prop_Send, "m_iTeamNum")>=0)
	{
	 ...
	 if (StrEqual(classname, "boomer") && waitingForBoomer && GetEntProp(entity, Prop_Send, "m_zombieClass") == 2) {
PrintToChatAll("don't stab me!");
